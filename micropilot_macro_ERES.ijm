...
// find best cell 
run("Select None"); run("Select best particle", "filter=threshold measurement=Mean threshold_min="+cell_intens_min+" threshold_max="+cell_intens_max);

if (selectionType() > -1) {

	// document the best cell
	selectWindow("eres"); run("Select None"); wait(500); run("Duplicate...", "title=eres_docu");
	run("Select best particle", "filter=threshold measurement=Mean threshold_min="+cell_intens_min+" threshold_max="+cell_intens_max);
